...
ProductsController.prototype.getPublished = function(callback) {
    this._data.getProducts({ published: true, ignored: false }, function(err, products) {
        callback(err && ('Can\'t load published product' + JSON.stringify(err)), products);
    });
};
...
ProductsController.prototype.getUnpublished = function(callback) {
    this._data.getProducts({ published: false, ignored: false }, function(err, products) {
        callback(err && ('Can\'t load unpublished product' + JSON.stringify(err)), products);
    });
};
...
ProductsController.prototype.getMissing = function(callback) {
    this._data.getProducts(function(err, products) {
        ...
        this._data.getContractorsProducts(function(err, contractorsProducts) {
            ...
            callback(err, catalog.checkAvailability(products, contractorsProducts).missing);
        });
    }.bind(this));
};
...
ProductsController.prototype.getAvailable = function(callback) {
    this._data.getProducts(function(err, products) {
        ...
        this._data.getContractorsProducts(function(err, contractorsProducts) {
            err && (err = 'Can\'t load missing product: ' + JSON.stringify(err));
            callback(err, catalog.checkAvailability(products, contractorsProducts).available);
        });
    }.bind(this));
};
...
ProductsController.prototype.getIgnored = function(callback) {
    this._data.getProducts({ ignored: true }, function(err, products) {
        callback(err && ('Can\'t load ignored product: ' + JSON.stringify(err)), products);
    });
};
